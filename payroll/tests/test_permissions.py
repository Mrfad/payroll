...
from payroll.permissions import IsOwnerOrAdmin, IsManagerOrDeveloper
...
class TestPermissions:
    ...
    def test_unauthenticated_blocked(self, api_client):
        response = api_client.get('/api/v1/payroll/companies/')
    ...
    def test_is_owner_or_admin_other_models(self):
        ...
        from payroll.models import AttendanceRecord, LeaveBalance, LeaveRequest, LeaveType
        from django.utils.timezone import now
        
        att = AttendanceRecord.objects.create(employee=employee, date=now().date(), first_in=now())
        request = APIRequestFactory().get('/')
        request.user = emp_user
        ...
        lr = LeaveRequest.objects.create(employee=employee, leave_type=lt, start_date='2025-01-01', end_date='2025-01-02', status='PENDING')
        request.user = emp_user
