...
class AttendanceProcessingService:
    ...
    def process_pending_logs(cls):
        ...
        """
        pending_logs = RawAttendanceLog.objects.filter(status="pending").order_by(
            "punch_time"
        )
        ...
        with transaction.atomic():
            for (external_id, device_type, date), logs in grouped_logs.items():
                ...
                if not employee:
                    # Mark as failed if no employee mapping exists
                    for log in logs:
                        log.status = "failed"
                ...
                if shift_assignment and shift_assignment.shift:
                    ...
                    if diff_in < -30:
                        ...
                        record.anomaly_reason = f"Punched in {abs(int(diff_in))} minutes early"
                    elif diff_in > 30:
                        record.is_anomaly = True
                        record.anomaly_reason = f"Punched in {int(diff_in)} minutes late"
                    ...
                    if not record.is_anomaly and record.last_out:
                        ...
                        if diff_out < -30:
                            record.is_anomaly = True
                            record.anomaly_reason = f"Punched out {abs(int(diff_out))} minutes early"
                        elif diff_out > 120:
                            record.is_anomaly = True
                            record.anomaly_reason = f"Punched out {int(diff_out)} minutes late"
