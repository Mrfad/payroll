...
def auth_client():
    user = User.objects.create_superuser(
        username="device_admin", password="password123"
    )
    client = APIClient()
...
def device_config(company):
    return DeviceConfiguration.objects.create(
        name="Test Device", device_type="zkteco", company=company
    )
...
class TestDeviceConfigurationViewSet:
    ...
    def test_create_device_config(self, auth_client, company):
        response = auth_client.post(
            ...
            {"name": "New Device", "device_type": "faceapi", "company": company.id},
        )
        assert response.status_code == status.HTTP_201_CREATED

    def test_update_device_config(self, auth_client, device_config):
        response = auth_client.patch(
            f"/api/v1/device/configurations/{device_config.id}/",
            {"name": "Updated Device"},
        )
        assert response.status_code == status.HTTP_200_OK
...
class TestDevicePushView:
    ...
    def test_push_unauthenticated(self):
        ...
        response = client.post(
            "/api/v1/device/push/",
            {"external_id": "EMP-001", "punch_time": now().isoformat()},
            format="json",
        )
    ...
    def test_push_missing_external_id(self, device_config):
        ...
        client.credentials(HTTP_X_DEVICE_TOKEN=str(device_config.api_token))
        response = client.post(
            "/api/v1/device/push/", {"direction": "in"}, format="json"
        )
