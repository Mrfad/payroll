# Backend\payroll\services\payroll.py
    ...
    def _calculate_employee_payroll(employee: Employee, run: PayrollRun):
        ...
        if base_salary > 0:
            ...
            details["earnings"].append(
                {"name": "Base Salary", "amount": float(base_salary)}
            )
        ...
        for struct in structures:
            ...
            if comp.component_type == "earning":
                ...
                details["earnings"].append({"name": comp.name, "amount": float(amount)})
            else:
                total_deductions += amount
                details["deductions"].append(
                    {"name": comp.name, "amount": float(amount)}
                )
        ...
        if total_ot_seconds > 0:
            ...
            details["attendance_summary"]["total_overtime_hours"] = float(
                round(ot_hours, 2)
            )

            # Look up the latest ShiftAssignment for this period to get the overtime multiplier
            shift_assignment = (
                ShiftAssignment.objects.filter(
                    employee=employee, start_date__lte=period.end_date
                )
                .order_by("-start_date")
                .first()
            )
            ...
            if shift_assignment and shift_assignment.overtime_policy:
                rate_multiplier = shift_assignment.overtime_policy.rate_multiplier

            ot_pay = ot_hours * hourly_rate * rate_multiplier
            if ot_pay > 0:
                gross_earnings += ot_pay
                details["earnings"].append(
                    {
                        "name": f"Overtime ({round(ot_hours, 2)} hours @ {rate_multiplier}x)",
