...
from payroll.models import AttendanceRecord, Employee, ShiftAssignment
        ...
        with transaction.atomic():
            for (external_id, device_type, date), logs in grouped_logs.items():
                ...
                if not employee:
                    ...
                    logger.info(f"Auto-creating employee for external_id='{external_id}' (device_type='{device_type}')")
                    from payroll.models import Company, Department
                    from django.contrib.auth.models import User
                    import uuid
                    
                    # Fallback to first company if device company is missing
                    company = logs[0].device.company if logs and logs[0].device else Company.objects.first()
                    
                    if company:
                        department, _ = Department.objects.get_or_create(company=company, name="Unassigned (From Device)")
                        ...
                        logger.warning(f"Cannot auto-create employee for {external_id} because no Company exists. Logs kept pending.")
