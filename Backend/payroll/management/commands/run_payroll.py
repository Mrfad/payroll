# Backend\payroll\management\commands\run_payroll.py
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "period_id",
            type=int,
            help="The ID of the PayrollPeriod to run calculation for",
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Starting payroll calculation for period {period.start_date} to {period.end_date}..."
        )

        try:
            run = PayrollCalculationService.calculate_payroll(period_id)
            self.stdout.write(
                self.style.SUCCESS(f"Successfully calculated payroll. Run ID: {run.id}")
            )
