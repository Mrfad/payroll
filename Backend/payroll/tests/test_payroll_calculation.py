# Backend\payroll\tests\test_payroll_calculation.py
...
def employee(company):
    user = User.objects.create(
        username="testemp", first_name="Test", last_name="Employee"
    )
...
@pytest.mark.django_db
def test_calculate_payroll_with_absences_and_overtime(
    company, employee, payroll_period
):
    # Add an allowance
...
def test_calculate_payroll_fallback_overtime_rate(company, employee, payroll_period):
    ...
    overtime_detail = next(
        (
            item
            for item in entry.details["earnings"]
            if item["name"].startswith("Overtime")
        ),
        None,
    )
