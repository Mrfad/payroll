# Backend\payroll\tests\test_views.py
import pytest
...
class TestEmployeeViewSet:
    def test_get_active_employees(self, auth_client, employee):
        ...
        # Act
        response = auth_client.get("/api/v1/employees/employees/")
    ...
    def test_retrieve_employee(self, auth_client, employee):
        response = auth_client.get(f"/api/v1/employees/employees/{employee.id}/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_create_employee(self, auth_client, company, department):
        ...
        response = auth_client.post(
            "/api/v1/employees/employees/", data=payload, format="json"
        )
    ...
    def test_get_deleted_employees(self, auth_client, employee):
        ...
        # Act
        response = auth_client.get("/api/v1/employees/employees/?show_deleted=true")
    ...
    def test_soft_delete_employee(self, auth_client, employee):
        # Act
        response = auth_client.delete(f"/api/v1/employees/employees/{employee.id}/")
    ...
    def test_restore_employee(self, auth_client, employee):
        ...
        # Act
        response = auth_client.post(
            f"/api/v1/employees/employees/{employee.id}/restore/?show_deleted=true"
        )
    ...
    def test_enroll_employee(self, auth_client, company, department):
        ...
        response = auth_client.post(
            "/api/v1/employees/employees/enroll/", data=payload, format="json"
        )
    ...
    def test_perform_update_freeze(self, auth_client, employee):
        ...
        response = auth_client.patch(
            f"/api/v1/employees/employees/{employee.id}/",
    ...
    def test_employee_punch_basic(self, auth_client, employee, company):
        # We need to test the /employees/{id}/punch/ endpoint
        response = auth_client.post(
            f"/api/v1/employees/employees/{employee.id}/punch/", format="json"
        )
        assert response.status_code == status.HTTP_201_CREATED
        ...
        device = DeviceConfiguration.objects.filter(
            company=company, name="App Punch"
        ).first()
        assert device is not None

        # Verify a raw log was created
        log = RawAttendanceLog.objects.filter(
            device=device, external_id=employee.employee_id
        ).first()
    ...
    def test_employee_punch(self, auth_client, employee, mocker):
        ...
        mock_process = mocker.patch(
            "attendance.services.AttendanceProcessingService.process_pending_logs"
        )

        response = auth_client.post(
            f"/api/v1/employees/employees/{employee.id}/punch/",
        ...
        log = RawAttendanceLog.objects.filter(
            device__name="App Punch", external_id=employee.employee_id
        ).first()
        assert log is not None
    ...
    def test_enroll_permission(self, auth_client, employee):
        response = auth_client.post(
            "/api/v1/employees/employees/enroll/",
            data={
...
class TestPayrollEntryViewSet:
    def test_payroll_entry_filtering_non_staff(self, employee):
        ...
        from payroll.models import PayrollEntry, PayrollPeriod, PayrollRun
...
class TestUserProfileViewSet:
    def test_user_profile_theme_broadcast(self, employee, mocker):
        ...
        response = client.patch(
            "/api/v1/accounts/user-profile/me/", data={"theme": "dark"}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
