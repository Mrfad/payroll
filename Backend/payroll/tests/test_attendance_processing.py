...
class TestAttendanceProcessingService:
    ...
    def test_process_anomalies(self, base_data):
        ...
        t1 = timezone.make_aware(timezone.datetime(today.year, today.month, today.day, 8, 15, 0))
        # Late punch out: 19:30 (+150 mins)
        t2 = timezone.make_aware(timezone.datetime(today.year, today.month, today.day, 19, 30, 0))
        ...
        AttendanceProcessingService.process_pending_logs()
        record = AttendanceRecord.objects.get(employee=base_data["employee"], date=today)
        assert record.is_anomaly is True
        assert "Punched out 150 minutes late" in record.anomaly_reason or "Punched in 45 minutes early" in record.anomaly_reason
        ...
        t3 = timezone.make_aware(timezone.datetime(today2.year, today2.month, today2.day, 9, 45, 0))
        t4 = timezone.make_aware(timezone.datetime(today2.year, today2.month, today2.day, 16, 15, 0))
        RawAttendanceLog.objects.create(
        ...
        AttendanceProcessingService.process_pending_logs()
        record2 = AttendanceRecord.objects.get(employee=base_data["employee"], date=today2)
        assert record2.is_anomaly is True
        assert "Punched out 45 minutes early" in record2.anomaly_reason or "Punched in 45 minutes late" in record2.anomaly_reason
    ...
    def test_process_statuses(self, base_data):
        ...
        # 1. Missing punch
        t1 = timezone.make_aware(timezone.datetime(today.year, today.month, today.day, 9, 0, 0))
        RawAttendanceLog.objects.create(
            device=base_data["device"], external_id="EMP123", punch_time=t1, raw_data={}
        )
        AttendanceProcessingService.process_pending_logs()
        record1 = AttendanceRecord.objects.get(employee=base_data["employee"], date=today)
        ...
        t2 = timezone.make_aware(timezone.datetime(today2.year, today2.month, today2.day, 9, 0, 0))
        t3 = timezone.make_aware(timezone.datetime(today2.year, today2.month, today2.day, 13, 0, 0))
        RawAttendanceLog.objects.create(
            device=base_data["device"], external_id="EMP123", punch_time=t2, raw_data={}
        )
        RawAttendanceLog.objects.create(
        ...
        t4 = timezone.make_aware(timezone.datetime(today3.year, today3.month, today3.day, 9, 0, 0))
        t5 = timezone.make_aware(timezone.datetime(today3.year, today3.month, today3.day, 11, 0, 0))
        RawAttendanceLog.objects.create(
            device=base_data["device"], external_id="EMP123", punch_time=t4, raw_data={}
        )
        RawAttendanceLog.objects.create(
        ...
        AttendanceProcessingService.process_pending_logs()
        record3 = AttendanceRecord.objects.get(employee=base_data["employee"], date=today3)
