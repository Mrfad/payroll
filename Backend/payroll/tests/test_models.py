# Backend\payroll\tests\test_models.py
import pytest
...
class TestEmployeeModel:
    def test_employee_creation_generates_employee_id(self):
        ...
        employee = Employee.objects.create(
            user=user, company=company, department=department
        )
...
class TestModelStringRepresentations:
    def test_str_methods(self):
        ...
        shift = Shift.objects.create(
            company=company, name="Morning", start_time="09:00:00", end_time="17:00:00"
        )
        assert str(shift) == "Morning - Str Company"

        bp = BreakPolicy.objects.create(
            company=company, name="Lunch", duration_minutes=60
        )
        assert str(bp) == "Lunch"

        op = OvertimePolicy.objects.create(
            company=company, name="Standard OT", daily_threshold_hours=8.0
        )
        ...
        sc = SalaryComponent.objects.create(
            company=company, name="Basic", component_type="earning"
        )
