...
class TestUserProfileViewSet:
    def test_list_user_profiles(self, auth_client):
        response = auth_client.get('/api/v1/payroll/user-profile/')
        assert response.status_code == status.HTTP_200_OK

    def test_user_profile_me_get(self, auth_client):
        response = auth_client.get('/api/v1/payroll/user-profile/me/')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_user_profile_me_patch(self, auth_client):
        ...
        response = auth_client.patch('/api/v1/payroll/user-profile/me/', {'theme': 'dark'}, format='json')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_user_profile_me_unauthenticated(self, api_client):
        response = api_client.get('/api/v1/payroll/user-profile/me/')
...
class TestAuditLogViewSet:
    def test_list_audit_logs(self, auth_client):
        ...
        AuditLog.objects.create(action='CREATE', target_user=user, details='Test Action')
        response = auth_client.get('/api/v1/payroll/audit-logs/')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_audit_log_target_user_filter(self, auth_client):
        ...
        response = auth_client.get(f'/api/v1/payroll/audit-logs/?target_user={user1.id}')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_audit_log_detail(self, auth_client):
        ...
        log = AuditLog.objects.create(action='DELETE', target_user=user, details='Delete Log')
        response = auth_client.get(f'/api/v1/payroll/audit-logs/{log.id}/')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_audit_log_non_manager_forbidden(self, api_client):
        ...
        response = api_client.get('/api/v1/payroll/audit-logs/')
